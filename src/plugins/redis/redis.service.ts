...
import { isObject } from '@src/utils/dateType';
...
export class RedisService {
  ...
  private getClient() {
    this.redisClient = new Redis({
      ...
      host: this.configService.get('REDIS_CONFIG.HOST'), // redisDb Redis host
      username: '', // needs Redis >= 6
      password: '', // 密码
      db: this.configService.get('REDIS_CONFIG.DB'), // redis是几个数据库的，使用第一个
    });
  }
  ...
  ): Promise<Result<number, ClientContext>> {
  ...
  async hget(
    key: string,
    field: string,
  ): Promise<Result<string | null, ClientContext>> {
  ...
  async hgetall(
    key: string,
  ): Promise<Result<Record<string, string>, ClientContext>> {
